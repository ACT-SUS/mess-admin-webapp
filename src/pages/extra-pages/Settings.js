  ...
  useEffect(() => {
    ...
    (async () => {
      const data = await axios.get(
        "http://localhost:5000/api/setting/semCycle"
      );
    ...
    <Grid container rowSpacing={4.5} columnSpacing={2.75}>
      <Grid item xs={12} sm={6} md={4} lg={4}>
        <MainCard title="Set Semester Cycle">
          ...
          />
          <p>{`Selected Cycle: ${formatDate(sem_cycle.start)} to ${formatDate(sem_cycle.end)}`}</p>
          <p>{`Current Cycle: ${formatDate(sem_cycle.start)} to ${formatDate(sem_cycle.end)}`}</p>
        </MainCard>
      </Grid>
                ...
                <CardContent>
                  ...
                  >
                    <Typography variant="h5" component="div">
                      Veg Dishes - ₹
                      {meal_prices["vegMeal"] ? meal_prices["vegMeal"] : 70}
                    </Typography>
                    <IconButton>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        (async () => {
                          (async () => {
                            await axios.put(
                              "http://localhost:5000/api/setting/prices",
                              newPrices
                            );
                            const data = await axios.get(
                              "http://localhost:5000/api/setting/prices"
